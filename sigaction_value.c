...
struct student
{
  ...
  timer_t t_id;
// signal handler 
//info 中 si_code的用法如下面所示  info中还有info-> si_ptr  还有 info-> si_int 分别信息传递的指针或者integer
  ...
  switch (info->si_code){ 
    ...
    case SI_QUEUE: printf("It was sent by a sigqueue, sending process (PID %i)\n",info->si_pid);
             //    printf("Additional value: %i\n",info->si_value.sival_int);
                    //使用结构体struct student* 解析 info  sival_ptr中携带了信息
...
int main(){
  ...
  //使用 myevent 和 真实时间创建一个timer 放入t_id中
...
---------------------------------
Parent (PID 24651) waits for a signal from child (PID 24652) 
Child process (PID 24652), will send a SIGTERM to parent
Signal with number 15 has arrived
Process (PID number 24652) sent the signal (calling kill)

---------------------------------
Child process ended
Signal with number 15 has arrived
It was sent by a sigqueue, sending process (PID 24652)
