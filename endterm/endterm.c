...
{
    pid_t Ursula , Csormester;
    signal(report ,handler);
    ...
    if(Ursula == 0)
    {
        ...
        allsick = allsick + sick;
        int real = num - sick;
        allvaccinated = allvaccinated + real;
        char csick[100] = "";
    ...
    {   
        ...
        sprintf(creal, "%d", real);
        char res[500]="";
        strcat(res,"I am Csormester.");
        strcat(res," ");
        strcat(res,csick);
        strcat(res," people sicked, and ");
        strcat(res,creal);
        strcat(res," people vaccinated ");
        printf("I am Csormester.I will have %s patients\n",sz);
        mq_send(mqdes,res,MSGSIZE,30);
        abort();
    }
    ...
    for (int i =1;i<=2;i++)
    {
        ...
        int size;
        char **ret = explode(' ', rcv_buf, &size);
        allsick = allsick + atoi(ret[3]);
        allvaccinated = allvaccinated + atoi(ret[7]);
        printf("%s\n",rcv_buf,prio);
        sleep(1);
    }
    ...
    time(&t);
    char callsick[100] ="";
    char cvaccinated[100] = "";
    char allpeople[100] = "";
    sprintf(callsick, "%d",allsick);
    sprintf(cvaccinated, "%d", allvaccinated);
    sprintf(allpeople, "%d", allsick+allvaccinated);

    char waittowrite[500] = "";
    strcat(waittowrite,ctime(&t));
    strcat(waittowrite," ");
    strcat(waittowrite," ===> ");
    strcat(waittowrite,allpeople);
    strcat(waittowrite," people came, ");
    strcat(waittowrite,callsick);
    strcat(waittowrite," of them were sick and ");
    strcat(waittowrite,cvaccinated);
    strcat(waittowrite," vaccinated\n");

    sem_wait(semid);
    write_to_file("data.txt",waittowrite);
    sem_post(semid);
   


    waitpid(Ursula, &st1, 0);
    waitpid(Csormester, &st2, 0);
